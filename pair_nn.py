...
def main():
    ...
    Options:
        ...
        --real-dataset      : TODO: add comment
        --real-dataset-root : TODO: add comment
    ...
    parser.add_argument('-K', type=int, default=10, help="set number K of NNs")
    ...
    for nn_metric in NN_METRICS:
        for nn_algorithm in NN_ALGORITHMS:
            ...
            if use_clip:
                ...
                nn_model_clip = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
            ...
            if use_openclip:
                ...
                nn_model_openclip = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
            ...
            if use_farl:
                ...
                nn_model_farl = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
            ...
            if use_dino:
                ...
                nn_model_dino = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
            ...
            if use_dinov2:
                ...
                nn_model_dinov2 = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
            ...
            if use_arcface:
                ...
                nn_model_arcface = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
            ...
            if use_deca_landmarks:
                ...
                nn_model_deca_landmarks = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
            ...
            if use_deca_angles:
                ...
                nn_model_deca_angles = NearestNeighbors(
                    n_neighbors=min(args.K, clip_fake_features.shape[0]) if args.K > 0 else clip_fake_features.shape[0],
                    algorithm=nn_algorithm,
                    metric=nn_metric
