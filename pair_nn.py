...
def main():
    """A script for finding the K Nearest Neighbors (NN) of each sample in a given real dataset from a pool of fake
    images (as generated by `create_fake_dataset.py`). The NNs will be found in all available features spaces (CLIP [1]
    and/or OpenCLIP [X] and/or FaRL [2] and/or DINO [3] and/or DINOv2 [Y] ArcFace [4] and/or DECA [5]) depending on
    their availability in the given real and fake datasets.
    ...
    parser.add_argument('--real-dataset', type=str, required=True, help="real dataset")
    parser.add_argument('--real-dataset-root', type=str, required=True,
                        help="set real dataset root directory")
    parser.add_argument('--fake-dataset-root', type=str, required=True,
                        help="set the fake dataset's root directory "
                             "(as generated by `create_fake_dataset.py` under datasets/)")
    parser.add_argument('-K', type=int, default=-1, help="set number K of NNs")
    ...
    # TODO
    #   /home/lab/code/FALCO_cvpr23/pair_nn.py:300: DeprecationWarning: Conversion of an array with ndim > 0 to a scalar is deprecated, and will error in future. Ensure you extract a single element from your array before performing this operation. (Deprecated NumPy 1.25.)
    ...
    for nn_metric in NN_METRICS:
        for nn_algorithm in NN_ALGORITHMS:
            ...
            if use_clip:
                ...
                nn_model_clip = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
            ...
            if use_openclip:
                ...
                nn_model_openclip = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                     algorithm=nn_algorithm,
                                                     metric=nn_metric
            ...
            if use_farl:
                ...
                nn_model_farl = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                 algorithm=nn_algorithm,
                                                 metric=nn_metric
            ...
            if use_dino:
                ...
                nn_model_dino = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                 algorithm=nn_algorithm,
                                                 metric=nn_metric
            ...
            if use_dinov2:
                ...
                nn_model_dinov2 = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                   algorithm=nn_algorithm,
                                                   metric=nn_metric
            ...
            if use_arcface:
                ...
                nn_model_arcface = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                    algorithm=nn_algorithm,
                                                    metric=nn_metric
            ...
            if use_deca_landmarks:
                ...
                nn_model_deca_landmarks = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                           algorithm=nn_algorithm,
                                                           metric=nn_metric
            ...
            if use_deca_angles:
                ...
                nn_model_deca_angles = NearestNeighbors(n_neighbors=args.K if args.K > 0 else clip_fake_features.shape[0],
                                                        algorithm=nn_algorithm,
                                                        metric=nn_metric
