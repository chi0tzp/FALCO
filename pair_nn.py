...
def main():
    ...
    with open(osp.join(args.real_dataset_root, 'features', 'image_filenames.txt')) as f:
    ...
    # === DECA features ===
    deca_real_features = None
    deca_real_features_file = osp.join(real_dataset_features_dir, 'deca_features.pt')
    use_deca = osp.exists(deca_real_features_file)
    if use_deca:
        deca_real_features = torch.load(deca_real_features_file).numpy()
        if args.verbose:
            print("  \\__DECA features: {}".format(deca_real_features.shape))
